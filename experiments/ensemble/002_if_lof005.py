...
def plot_diversity(
    ...
    fig.suptitle("앙상블 002 — 모델 다양성 진단 (IF-010 × LOF-005)", fontsize=13)
    ...
    ax.scatter(if_val[normal_mask][samp], lof_val[normal_mask][samp],
    ...
    bw = 0.25
    b_if  = ax.bar(x - bw, if_auprs,  bw, label="IF",       color="steelblue", alpha=0.8)
    b_lof = ax.bar(x,      lof_auprs, bw, label="LOF-005",  color="orange",    alpha=0.8)
    b_ens = ax.bar(x + bw, ens_auprs, bw, label="Ensemble", color="seagreen",  alpha=0.8)

    for bars in [b_if, b_lof, b_ens]:
        for bar in bars:
            h = bar.get_height()
            if not np.isnan(h):
    ...
    ax.set_ylim(0, min(1.25, max(filter(lambda v: not np.isnan(v),
                                        if_auprs + lof_auprs + ens_auprs)) * 1.35))
    ax.set_title("Val 유형별 AUPR 비교 (IF / LOF-005 / Ensemble)", fontsize=10)
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    print("=== 앙상블 002: IF-010 + LOF-005 (Rank 평균) ===\n")
    ...
    print(f"\n  [Val] 유형별 AUPR")
    print(f"  {'유형':<14} {'IF':>8} {'LOF-005':>9} {'Ensemble':>10}")
    print("  " + "-" * 44)
    for type_name, lo, hi in [
        ...
        print(f"  {type_name:<14} {_fmt(ifa):>8} {_fmt(lfa):>9} {_fmt(ensa):>10}")
    ...
    plot_diversity(
        ...
        ens_val_scores,
        save_path=str(OUTPUT_DIR / "002_diversity.png"),
    ...
    plot_score_compare(
        ...
        val_labels,
        save_path=str(OUTPUT_DIR / "002_score_trace.png"),
    )

    plot_full(viz_ens_val, val_labels, viz_ens_test, test_labels,
              tag="002_ensemble_if_lof005",
              save_path=str(OUTPUT_DIR / "002_val_score_trace.png"))
    plot_zooms(viz_ens_val, val_labels, viz_ens_test, test_labels,
               tag="002_ensemble_if_lof005",
               save_path=str(OUTPUT_DIR / "002_val_score_zoom.png"))
    plot_score_hist(viz_ens_val, val_labels, viz_ens_test, test_labels,
                    tag="002_ensemble_if_lof005",
                    save_path=str(OUTPUT_DIR / "002_score_hist.png"))
