"""앙상블 007: LOF-012 + GMM-005 — Rank 평균 앙상블

모델 조합
---------
  LOF: x_f8 차분 → 연속형 7채널 원본 → StandardScaler → LOF-012 기반 파이프라인
  GMM: x_f8 차분 → 연속형 7채널 → StandardScaler → PCA(95%)
       → Rolling(W=20, mean/std/min/max/range) → GMM-005 기반 파이프라인

설계 근거
---------
  LOF-012는 LOF-005에 x_f8 차분을 추가한 실험입니다.
  GMM-005는 동일한 x_f8 차분 후 PCA 압축 공간에서 확률 밀도 이상을 탐지합니다.
  두 모델 모두 x_f8 차분을 적용하지만, LOF는 원시 7채널/GMM은 PCA+롤링 통계를 사용하여
  표현 공간이 서로 다릅니다.
...
출력 파일
---------
  007_diversity.png       — Score 산점도 / 상관계수 히트맵 / AUPR 비교 / Score KDE
  007_score_trace.png     — val: LOF / GMM / Ensemble 3행 score 추이
  007_val_score_trace.png — Ensemble만: val+test 전체 추이
  007_val_score_zoom.png  — Ensemble: 이상 구간 확대
  007_score_hist.png      — Ensemble: score 분포 히스토그램
"""
...
def plot_diversity(
    ...
    fig.suptitle("앙상블 007 — 모델 다양성 진단 (LOF-012 × GMM-005)", fontsize=13)
    ...
    bw = 0.25
    b_lof = ax.bar(x - bw, lof_auprs, bw, label="LOF-012", color="steelblue", alpha=0.8)
    ...
    ax.set_title("Val 유형별 AUPR 비교 (LOF-012 / GMM-005 / Ensemble)", fontsize=10)
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    print("=== 앙상블 007: LOF-012 + GMM-005 (Rank 평균) ===\n")
    ...
    print(f"\n[다양성] LOF-012 ↔ GMM-005 score 상관계수")
    ...
    print(f"\n  [Val] 유형별 AUPR")
    print(f"  {'유형':<14} {'LOF-012':>9} {'GMM-005':>9} {'Ensemble':>10}")
    print("  " + "-" * 45)
    ...
    plot_diversity(
        ...
        ens_val_scores,
        save_path=str(OUTPUT_DIR / "007_diversity.png"),
    ...
    plot_score_compare(
        ...
        val_labels,
        save_path=str(OUTPUT_DIR / "007_score_trace.png"),
    )

    plot_full(viz_ens_val, val_labels, viz_ens_test, test_labels,
              tag="007_ensemble_lof012_gmm005",
              save_path=str(OUTPUT_DIR / "007_val_score_trace.png"))
    plot_zooms(viz_ens_val, val_labels, viz_ens_test, test_labels,
               tag="007_ensemble_lof012_gmm005",
               save_path=str(OUTPUT_DIR / "007_val_score_zoom.png"))
    plot_score_hist(viz_ens_val, val_labels, viz_ens_test, test_labels,
                    tag="007_ensemble_lof012_gmm005",
                    save_path=str(OUTPUT_DIR / "007_score_hist.png"))
