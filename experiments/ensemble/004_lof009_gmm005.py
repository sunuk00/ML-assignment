"""앙상블 004: LOF-009 + GMM-005 — Rank 평균 앙상블

모델 조합
---------
  LOF: 연속형 7채널 → Window Stats(W=3, mean/std/min/max/median) → StandardScaler
       → LOF-009 기반 파이프라인
...
설계 근거
---------
  LOF-009는 짧은 윈도우(W=3) 슬라이딩 통계로 국소 밀도 이상을 탐지합니다.
  GMM-005는 x_f8 트렌드 제거 후 PCA 압축된 공간에서 확률 밀도 이상을 탐지합니다.
  두 모델이 서로 다른 표현 공간을 사용하므로 다양성이 높을 것으로 기대합니다.
...
출력 파일
---------
  004_diversity.png       — Score 산점도 / 상관계수 히트맵 / AUPR 비교 / Score KDE
  004_score_trace.png     — val: LOF / GMM / Ensemble 3행 score 추이
  004_val_score_trace.png — Ensemble만: val+test 전체 추이
  004_val_score_zoom.png  — Ensemble: 이상 구간 확대
  004_score_hist.png      — Ensemble: score 분포 히스토그램
"""
...
from src.features      import DISCRETE_COLS, window_features, rolling_features
...
# ── LOF 하이퍼파라미터 (LOF-009 기반) ───────────────────────────
LOF_NEIGHBORS  = 10
LOF_CONTAM     = 0.0001
LOF_WINDOW     = 3
LOF_STATS      = ["mean", "std", "min", "max", "median"]
...
def build_lof_features(
    ...
    train_feats = window_features(train_df, cols=cont_cols, window_size=LOF_WINDOW, stats=LOF_STATS)
    val_feats   = window_features(val_df,   cols=cont_cols, window_size=LOF_WINDOW, stats=LOF_STATS)
    test_feats  = window_features(test_df,  cols=cont_cols, window_size=LOF_WINDOW, stats=LOF_STATS)
    scaler      = fit_scaler(train_feats)
    return (
        apply_scaler(scaler, train_feats).to_numpy(),
        apply_scaler(scaler, val_feats).to_numpy(),
        apply_scaler(scaler, test_feats).to_numpy(),
    )
...
def plot_diversity(
    ...
    fig.suptitle("앙상블 004 — 모델 다양성 진단 (LOF-009 × GMM-005)", fontsize=13)
    ...
    bw = 0.25
    b_lof = ax.bar(x - bw, lof_auprs, bw, label="LOF-009", color="steelblue", alpha=0.8)
    ...
    ax.set_title("Val 유형별 AUPR 비교 (LOF-009 / GMM-005 / Ensemble)", fontsize=10)
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    print("=== 앙상블 004: LOF-009 + GMM-005 (Rank 평균) ===\n")
    ...
    print(f"  LOF feature dim: {lof_train_X.shape[1]}"
          f"  (cont {len(cont_cols)}×{len(LOF_STATS)} window stats, W={LOF_WINDOW} + scaler)")
    ...
    print(f"\n[다양성] LOF-009 ↔ GMM-005 score 상관계수")
    ...
    for name, vs, vl, ts, tl in [
        ("LOF-009",  lof_val_scores, val_labels,  lof_test_scores, test_labels),
    ...
    print(f"\n  [Val] 유형별 AUPR")
    print(f"  {'유형':<14} {'LOF-009':>9} {'GMM-005':>9} {'Ensemble':>10}")
    print("  " + "-" * 45)
    ...
    plot_diversity(
        ...
        ens_val_scores,
        save_path=str(OUTPUT_DIR / "004_diversity.png"),
    ...
    plot_score_compare(
        {
            f"LOF-009 (window W={LOF_WINDOW} stats + scaler)":          viz_lof_val,
            f"GMM-005 (x_f8 diff + PCA + rolling W={GMM_WINDOW})":      viz_gmm_val,
            "Ensemble (rank 평균)":                                       viz_ens_val,
        },
        val_labels,
        save_path=str(OUTPUT_DIR / "004_score_trace.png"),
    )

    plot_full(viz_ens_val, val_labels, viz_ens_test, test_labels,
              tag="004_ensemble_lof009_gmm005",
              save_path=str(OUTPUT_DIR / "004_val_score_trace.png"))
    plot_zooms(viz_ens_val, val_labels, viz_ens_test, test_labels,
               tag="004_ensemble_lof009_gmm005",
               save_path=str(OUTPUT_DIR / "004_val_score_zoom.png"))
    plot_score_hist(viz_ens_val, val_labels, viz_ens_test, test_labels,
                    tag="004_ensemble_lof009_gmm005",
                    save_path=str(OUTPUT_DIR / "004_score_hist.png"))
