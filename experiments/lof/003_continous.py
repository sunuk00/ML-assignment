...
연속형 채널(7개)에만 원본 값을 사용하여 피처로 사용합니다.
...
if __name__ == "__main__":
    ...
    # 4. 차분 없이 원본값 사용 (연속형만)
    train_X = train_df[cont_cols].to_numpy()
    val_X   = val_df[cont_cols].to_numpy()
    test_X  = test_df[cont_cols].to_numpy()
    print(f"  feature dim: {train_X.shape[1]}  ({len(cont_cols)} raw_cont)")
    ...
    plot_full(val_scores, val_labels, test_scores, test_labels,
              tag="003_lof_only_cont", save_path=str(OUTPUT_DIR / "003_val_score_trace.png"))
    plot_zooms(val_scores, val_labels, test_scores, test_labels,
               tag="003_lof_only_cont", save_path=str(OUTPUT_DIR / "003_val_score_zoom.png"))
    plot_score_hist(val_scores, val_labels, test_scores, test_labels,
                    tag="003_lof_only_cont", save_path=str(OUTPUT_DIR / "003_score_hist.png"))