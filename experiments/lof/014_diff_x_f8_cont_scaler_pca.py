"""실험 014: LOF — x_f8 차분 + 연속형 원본 + StandardScaler + PCA

012 파이프라인에 PCA를 추가합니다.
x_f8 채널에만 1차 차분(diff)을 적용하여 장기 트렌드를 제거한 뒤,
연속형 7채널 원본에 StandardScaler → PCA(95% 분산 유지)를 순서대로 적용하여
LOF로 이상치 점수를 산출합니다.

피처 차원: 연속형 7채널 → StandardScaler → PCA(95%)
"""
...
if __name__ == "__main__":
    ...
    print("=== 014 LOF — x_f8 Diff + Raw(Cont) + StandardScaler + PCA ===\n")
    ...
    print(f"  PCA components: {train_pca.shape[1]}  (variance={PCA_VARIANCE:.2f}, 입력 {train_scaled.shape[1]}차원)")

    # 7. 피처 준비
    train_X = train_pca.to_numpy()
    val_X   = val_pca.to_numpy()
    test_X  = test_pca.to_numpy()
    print(f"  feature dim: {train_X.shape[1]}  (cont {len(cont_cols)}ch → scaler → PCA)")
    ...
    plot_full(val_scores, val_labels, test_scores, test_labels,
              tag="014_lof_diff_x_f8_cont_scaler_pca",
              save_path=str(OUTPUT_DIR / "014_val_score_trace.png"))
    plot_zooms(val_scores, val_labels, test_scores, test_labels,
               tag="014_lof_diff_x_f8_cont_scaler_pca",
               save_path=str(OUTPUT_DIR / "014_val_score_zoom.png"))
    plot_score_hist(val_scores, val_labels, test_scores, test_labels,
                    tag="014_lof_diff_x_f8_cont_scaler_pca",
                    save_path=str(OUTPUT_DIR / "014_score_hist.png"))
