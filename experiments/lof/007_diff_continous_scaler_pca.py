"""실험 007: LOF — 차분(연속형) + StandardScaler + PCA

연속형 채널(7개)에 차분을 먼저 적용한 뒤, 학습 데이터 기준 StandardScaler와 PCA를
순서대로 적용하여 피처로 사용합니다.
이산형 채널(3개)은 포함하지 않습니다.
LocalOutlierFactor(novelty=True)로 학습 및 추론합니다.

피처 차원: 연속형 7(diff) -> scaler -> PCA
"""
...
if __name__ == "__main__":
    ...
    print("=== 007 LOF — Diff(Cont) + StandardScaler + PCA ===\n")
    ...
    # 5. 학습 데이터 기준 StandardScaler 적용
    train_cont = train_diff[cont_cols]
    val_cont   = val_diff[cont_cols]
    test_cont  = test_diff[cont_cols]
    ...
    # 7. 피처: 차분 연속형 + StandardScaler + PCA
    train_X = train_pca.to_numpy()
    val_X   = val_pca.to_numpy()
    test_X  = test_pca.to_numpy()
    print(f"  feature dim: {train_X.shape[1]}  (diff_cont -> scaler -> pca)")
    ...
    plot_full(val_scores, val_labels, test_scores, test_labels,
              tag="007_lof_diff_cont_scaler_pca", save_path=str(OUTPUT_DIR / "007_val_score_trace.png"))
    plot_zooms(val_scores, val_labels, test_scores, test_labels,
               tag="007_lof_diff_cont_scaler_pca", save_path=str(OUTPUT_DIR / "007_val_score_zoom.png"))
    plot_score_hist(val_scores, val_labels, test_scores, test_labels,
                    tag="007_lof_diff_cont_scaler_pca", save_path=str(OUTPUT_DIR / "007_score_hist.png"))