"""실험 004: LOF — 원본(연속형 + 차분)

연속형 채널(7개)에 차분을 사용하여 피처로 사용합니다.
...
if __name__ == "__main__":
    ...
    # 5. 피처: 차분 연속형
    train_X = train_diff[cont_cols].to_numpy()
    val_X   = val_diff[cont_cols].to_numpy()
    test_X  = test_diff[cont_cols].to_numpy()
    print(f"  feature dim: {train_X.shape[1]}  ({len(cont_cols)} diff_cont)")
    ...
    plot_full(val_scores, val_labels, test_scores, test_labels,
              tag="004_lof_only_diff", save_path=str(OUTPUT_DIR / "004_val_score_trace.png"))
    plot_zooms(val_scores, val_labels, test_scores, test_labels,
               tag="004_lof_only_diff", save_path=str(OUTPUT_DIR / "004_val_score_zoom.png"))
    plot_score_hist(val_scores, val_labels, test_scores, test_labels,
                    tag="004_lof_only_diff", save_path=str(OUTPUT_DIR / "004_score_hist.png"))